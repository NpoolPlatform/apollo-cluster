pipeline {
  ...
  stages {
    ...
    stage('Override values.portal.yaml') {
      steps {
        sh 'echo "portaldb:\n" >> ./values.portal.yaml'
        sh "echo \"  host: `curl http://consul-server.kube-system.svc.cluster.local:8500/v1/agent/health/service/name/mysql.npool.top | jq '.[0] | .Service | .Address'`\" >> ./values.portal.yaml"
