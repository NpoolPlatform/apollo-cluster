  ...
  stages {
    ...
    stage('Build apollo image') {
      ...
      steps {
        ...
        sh 'cd service-config; docker build -t uhub.service.ucloud.cn/entropypool/apollo-configservice:1.9.1 .'
        sh 'mkdir -p service-admin/.docker-tmp; cp /usr/bin/consul service-admin/.docker-tmp'
        sh 'cd service-admin; docker build -t uhub.service.ucloud.cn/entropypool/apollo-adminservice:1.9.1 .'
        sh 'mkdir -p service-portal/.docker-tmp; cp /usr/bin/consul service-portal/.docker-tmp'
        sh 'cd service-portal; docker build -t uhub.service.ucloud.cn/entropypool/apollo-portal:1.9.1 .'
      }
    }
    ...
    stage('Release apollo image') {
      ...
      steps {
        sh(returnStdout: true, script: '''
          set +e
          while true; do
            docker push uhub.service.ucloud.cn/entropypool/apollo-configservice:1.9.1
            if [ $? -eq 0 ]; then
              break
            fi
            sleep 5
          ...
          while true; do
            docker push uhub.service.ucloud.cn/entropypool/apollo-portal:1.9.1
            if [ $? -eq 0 ]; then
              break
            fi
            sleep 5
          ...
          while true; do
            docker push uhub.service.ucloud.cn/entropypool/apollo-adminservice:1.9.1
            if [ $? -eq 0 ]; then
              break
            fi
            sleep 5
